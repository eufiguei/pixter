...
export async function GET(request: Request) {
  try {
    ...
    if (profileError) {
      console.error("Error fetching profile: from api/motorista/payment", profileError);
    ...
    try {
      ...
      try {
        ...
        console.log("Retrieved Stripe account2:", account.id,account.email,account.country,account.metadata);
      ...
      console.log("Stripe balance retrieved successfully",bal);
