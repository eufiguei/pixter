-- Supabase SQL for Temporary Payment Storage (Task 3.5)
...
CREATE TABLE public.temporary_payments (
  ...
  expires_at TIMESTAMPTZ GENERATED ALWAYS AS (created_at + INTERVAL '15 minutes') STORED
);

-- Add indexes for efficient querying and cleanup
CREATE INDEX idx_temporary_payments_ip_address ON public.temporary_payments (ip_address);
CREATE INDEX idx_temporary_payments_expires_at ON public.temporary_payments (expires_at);
...
-- This job runs every minute to delete expired entries.
